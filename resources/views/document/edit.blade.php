<x-layout>
    <div class="mb-6 flex items-center gap-3">
        ...
        <div>
            <h2 class="text-2xl font-extrabold text-slate-800 tracking-tight">Edit Document</h2>
            <p class="text-sm text-slate-500 mt-1">Update document metadata or replace file</p>
        </div>
    </div>
            ...
            <form action="{{ route('documents.update', $document->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="space-y-5 mb-6">
                    <div class="form-control w-full">
                        ...
                        <input type="text" name="title" value="{{ old('title', $document->title) }}" placeholder="Enter document title" class="input input-bordered h-11 w-full bg-slate-50 border-slate-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-all rounded-lg text-sm @error('title') border-rose-300 focus:ring-rose-100 focus:border-rose-500 @enderror" required />
                        @error('title')
                    ...
                    <div class="form-control w-full">
                        ...
                        <textarea name="description" placeholder="Enter brief description (optional)" class="textarea textarea-bordered min-h-[100px] w-full bg-slate-50 border-slate-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-all rounded-lg text-sm leading-relaxed @error('description') border-rose-300 focus:ring-rose-100 focus:border-rose-500 @enderror">{{ old('description', $document->description) }}</textarea>
                        @error('description')
                            <label class="label pt-1 pb-0"><span class="label-text-alt text-rose-500 font-medium">{{ $message }}</span></label>
                        @enderror
                    </div>
                    ...
                    <div class="form-control w-full">
                        <label class="label pb-1.5 flex justify-between items-end">
                            <span class="label-text text-sm font-semibold text-slate-700">Replace PDF File (Optional)</span>
                            <span class="text-[11px] font-medium text-slate-400 bg-slate-100 px-2 py-0.5 rounded">Max: 10MB</span>
                        </label>
                        <input type="file" name="file" accept=".pdf" class="file-input file-input-bordered h-11 w-full bg-slate-50 border-slate-200 focus:border-blue-500 transition-all rounded-lg text-sm @error('file') file-input-error border-rose-300 @enderror" />
                        <label class="label pt-1.5 pb-0">
                            <span class="label-text-alt text-slate-500"><i class="bi bi-info-circle mr-1"></i> Leave empty to keep existing file.</span>
                        </label>
                        @error('file')
                            <label class="label pt-1 pb-0"><span class="label-text-alt text-rose-500 font-medium">{{ $message }}</span></label>
                        @enderror
