<x-layout>
    <x-breadcrumb :items="[
        ...
        ['label' => $category->name, 'url' => route('categories.index')],
        ['label' => 'Edit'],
    ]" />
    ...
    <div class="mb-6 flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-extrabold text-slate-800 tracking-tight">Edit Category</h1>
            <p class="text-sm text-slate-500 mt-1 truncate max-w-xs">{{ $category->name }}</p>
        </div>
    </div>
                ...
                <div class="space-y-5 mb-6">
                    ...
                    <div class="form-control">
                        ...
                        <textarea name="description" rows="4" placeholder="Brief explanation of what this category contains..."
                            class="textarea textarea-bordered w-full bg-slate-50 border-slate-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-all rounded-lg text-sm leading-relaxed @error('description') border-rose-400 bg-rose-50 @enderror">{{ old('description', $category->description) }}</textarea>
                        @error('description')
                            <label class="label pt-1 pb-0"><span class="label-text-alt text-rose-500 font-medium">{{ $message }}</span></label>
                        @enderror
                    </div>
